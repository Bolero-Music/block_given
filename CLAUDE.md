...
ALCHEMY_API_KEY=... bin/console # IRB with Vium configured (VIUM_CHAIN, VIUM_ABI_PATH)
```
...
| Path | Role |
|---|---|
| `lib/vium.rb` | requires, `Vium.configure / config / client / watchers / reset!` |
| `lib/vium/configuration.rb` | global defaults (polling interval, timeout, multipliers, abi_path, logger) |
| `lib/vium/chain.rb` | `Chain` struct + `Chains` constants and `resolve` |
| `lib/vium/connectors/` | `Base` (interface), `Http` (Net::HTTP, retries, `redact`), `Alchemy`, `Stub` (tests) |
| `lib/vium/client.rb` | JSON-RPC methods, fee estimation, receipts, logs, `watch_*` |
| `lib/vium/poller.rb` | `Poller.poll` (blocking) and `Watcher` (thread + registry by id) |
| `lib/vium/wallet.rb` | key, signing (EIP-1559/legacy/191/712), tx preparation |
| `lib/vium/transaction.rb`, `receipt.rb`, `event.rb`, `normalizer.rb` | value objects around RPC results |
| `lib/vium/abi/` | `Interface` (parse, overloads), `Function`, `Event`, `CustomError`, `Parameter`, `Coder` (coercion) |
| `lib/vium/contract.rb` | class-level DSL (`abi`, `abi_file`, `address`, `chain`) + read/write/simulate/events |
| `lib/vium/railtie.rb` | optional, loaded when `Rails::Railtie` is defined |
...
- **No network in specs.** RPC via `Vium::Connectors::Stub` (`Stub.sequence` for successive answers, procs for
...
- **No ABI in the gem.** Applications own their ABIs (`abi_file`, `Vium.config.abi_path`).
...
- **Errors are typed.** Raise `Vium::*` errors (`errors.rb`); wrap third-party exceptions (`Eth::Abi::*`,
...
- Today the Nest.js blockchain service watches on-chain events; vium's watchers + app-side cursor are the
