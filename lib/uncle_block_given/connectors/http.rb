...
module UncleBlockGiven
  module Connectors
    ...
    #
    #   UncleBlockGiven::Connectors::Http.new(url: "http://127.0.0.1:8545")
    #   UncleBlockGiven::Connectors::Http.new # -> falls back to chain.rpc_urls.first
    class Http < Base
      ...
      def initialize(url: nil, headers: {}, timeout: 30, retries: 3, retry_delay: 0.5, logger: nil)
        super()
        @url = url
        user_agent = "uncle_block_given/#{UncleBlockGiven::VERSION}"
      ...
      def with_retries(method)
        ...
        loop do
          ...
          logger.debug { "[uncle_block_given] retrying #{method} (attempt #{attempt}/#{retries}) in #{delay}s" }
          sleep(delay)
        end
      end
