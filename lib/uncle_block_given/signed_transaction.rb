...
module UncleBlockGiven
  ...
  #   signed.broadcast                                  # => UncleBlockGiven::Transaction
  #
  ...
  class SignedTransaction
    ...
    def broadcast
      ...
      unless sent.hash.to_s.casecmp?(hash)
        client.logger.warn do
          "[uncle_block_given] node returned #{sent.hash} for signed transaction #{hash}"
        end
      end
      transaction
    end
    ...
    def inspect = "#<UncleBlockGiven::SignedTransaction #{hash} nonce=#{nonce} to=#{to}>"
