...
module UncleBlockGiven
  ...
  #
  #   receipt = UncleBlockGiven::Poller.poll(interval: 2, timeout: 120) { client.get_transaction_receipt(hash) }
  #
  # The block is called until it returns a non-nil / non-false value, which is
  # returned. Raises UncleBlockGiven::TimeoutError when the timeout elapses.
  module Poller
    module_function
  ...
  #   UncleBlockGiven.watchers                     # => [#<UncleBlockGiven::Watcher blocks ...>]
  #   UncleBlockGiven::Watcher.find("blocks").stop
  #   UncleBlockGiven::Watcher.stop_all
  class Watcher
    @registry = {}
    ...
    def start
      ...
      @thread.name = "uncle_block_given:#{id}"
    ...
    def inspect
      ...
      "#<UncleBlockGiven::Watcher #{id} #{name} #{status} cursor=#{cursor.inspect} ticks=#{ticks}#{error}>"
    end
    ...
    def handle_error(error)
      ...
      if @on_error
        ...
        logger.warn { "[uncle_block_given] watcher #{id}: #{error.class}: #{error.message}" }
      end
    end
