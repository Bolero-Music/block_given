...
module UncleBlockGiven
  ...
  #     abi_file "abis/erc20.json"   # your app's ABI file (see UncleBlockGiven.config.abi_path)
  #     address "0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913"
  #   end
  #
  #   usdc = Usdc.new(wallet: UncleBlockGiven::Wallet.new(private_key: "0x..."))
  #   usdc.balance_of(wallet.address)                       # eth_call, decoded
  #   tx = usdc.transfer(to: "0x...", value: 1e6)           # signed + broadcast -> UncleBlockGiven::Transaction
  #   tx.wait!                                              # polls the receipt
  #
  # Transaction / call overrides go in the reserved `tx:` keyword:
  #   vault.deposit(amount, tx: { value: UncleBlockGiven::Utils.parse_ether("0.1"), gas: 200_000 })
  ...
  class Contract
    ...
    class << self
      ...
      # UncleBlockGiven.config.abi_path when set (ABIs live in your app, not in the gem).
      def abi_file(path)
        base = UncleBlockGiven.config.abi_path
      ...
      def define_abi_methods!
        interface.function_names.each do |ruby_name|
          if Contract.method_defined?(ruby_name) || Contract.private_method_defined?(ruby_name)
            UncleBlockGiven.config.logger.debug do
              "[uncle_block_given] #{name}: skipping ##{ruby_name} (reserved), use read/write"
            end
            next
          end
    ...
    # result. Raises UncleBlockGiven::ContractRevertError with the decoded reason on failure.
    ...
    # Polls for new events in a background thread. Returns a UncleBlockGiven::Watcher.
