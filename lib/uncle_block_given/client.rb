...
module UncleBlockGiven
  ...
  #
  #   connector = UncleBlockGiven::Connectors::Alchemy.new(api_key: "...")
  #   client = UncleBlockGiven::Client.new(chain: :base, connector: connector)
  #   client.block_number
  ...
  class Client
    ...
    def initialize(chain: nil, connector: nil, polling_interval: nil, timeout: nil, logger: nil)
      @chain = chain ? Chains.resolve(chain) : UncleBlockGiven.config.chain!
      @connector = connector || UncleBlockGiven.config.connector!
      @polling_interval = polling_interval
      @timeout = timeout
      @logger = logger
    end

    def polling_interval = @polling_interval || UncleBlockGiven.config.polling_interval
    def timeout = @timeout || UncleBlockGiven.config.timeout
    def logger = @logger || UncleBlockGiven.config.logger

    # Raw JSON-RPC call: client.request("eth_blockNumber") / client.request("eth_getBalance", addr, "latest")
    def request(method, *params)
      logger.debug { "[uncle_block_given] -> #{method} #{params.inspect}" }
      result = connector.request(method, params, chain: chain)
      logger.debug { "[uncle_block_given] <- #{method} #{result.inspect[0, 200]}" }
      result
    end
    ...
    def estimate_fees_per_gas(base_fee_multiplier: nil)
      multiplier = base_fee_multiplier || UncleBlockGiven.config.base_fee_multiplier
    ...
    def wait_for_transaction_receipt(hash, confirmations: nil, timeout: nil, polling_interval: nil)
      confirmations ||= UncleBlockGiven.config.confirmations
    ...
    def get_logs_in_chunks(from_block:, address: nil, topics: nil, to_block: :latest, max_block_range: nil)
      size = max_block_range || UncleBlockGiven.config.max_block_range
    ...
    #   id:              stable identifier for UncleBlockGiven::Watcher.find / stop (default: auto-generated)
    def watch_logs(address: nil, topics: nil, from_block: nil, polling_interval: nil, max_block_range: nil,
      ...
      watcher(name || "logs@#{Array(address).first || '*'}", polling_interval, id: id) do |watcher|
        ...
        while last < head && !watcher.stopped?
          to = [last + (max_block_range || UncleBlockGiven.config.max_block_range), head].min
    ...
    def inspect = "#<UncleBlockGiven::Client chain=#{chain} connector=#{connector.inspect}>"
