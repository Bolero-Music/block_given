...
module UncleBlockGiven
  ...
  #   wallet.address
  #   wallet.send_transaction(to: "0x...", value: UncleBlockGiven::Utils.parse_ether("0.01")).wait
  class Wallet
    attr_reader :address
    ...
    def client = @client || UncleBlockGiven.client
    ...
    def to_s = address
    def inspect = "#<UncleBlockGiven::Wallet #{address}>"
    ...
    def estimate_gas(to:, data:, value:)
      ...
      (estimate * UncleBlockGiven.config.gas_multiplier).ceil
    end
      ...
      when Float, BigDecimal, Rational
        unless value == value.to_i
          raise InvalidArgumentError,
                "value must be an integer amount of wei (use UncleBlockGiven::Utils.parse_ether)"
        end
