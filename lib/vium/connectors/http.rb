...
module Vium
  module Connectors
    ...
    #
    #   Vium::Connectors::Http.new(url: "http://127.0.0.1:8545")
    #   Vium::Connectors::Http.new # -> falls back to chain.rpc_urls.first
    class Http < Base
      ...
      def initialize(url: nil, headers: {}, timeout: 30, retries: 3, retry_delay: 0.5, logger: nil)
        super()
        @url = url
        @headers = { "Content-Type" => "application/json", "User-Agent" => "vium/#{Vium::VERSION}" }.merge(headers)
        @timeout = timeout
        @retries = retries
      ...
      def with_retries(method)
        ...
        loop do
          ...
          logger.debug { "[vium] retrying #{method} (attempt #{attempt}/#{retries}) in #{delay}s" }
          sleep(delay)
        end
      end
