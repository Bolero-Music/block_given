---
name: release
description: Cut a block_given release — version bump, CHANGELOG, verification matrix, tag, RubyGems publication via trusted publishing or rake release.
---

# Release

1. **Decide the version** (semver). Breaking public API → major; new API → minor; fixes → patch.
2. **Update** `lib/block_given/version.rb`.
...
RubyGems → gem `block_given` → Trusted publishers → GitHub Actions: repository `Bolero-Music/block_given`, workflow
