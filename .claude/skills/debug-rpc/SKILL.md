---
name: debug-rpc
description: Investigate a failing contract call, transaction, or event fetch (revert reasons, wrong encoding, provider limits, rate limits, unexpected nil). Use when a BlockGiven user reports "it does not work against the node".
---
...
```ruby
BlockGiven.config.logger = Logger.new($stdout, level: Logger::DEBUG)   # logs every method + params + result
```
Keys are masked; the debug log is safe to paste. Compare `data` against `contract.encode_function_data(...)`.

## 2. Reproduce offline

Copy the real response into a `BlockGiven::Connectors::Stub` and write the failing spec first. Stub values:
...
```ruby
BlockGiven.configure { |c| c.connector = BlockGiven::Connectors::Http.new; c.chain = :base }   # public RPC, no key
BlockGiven.client.block_number
```
