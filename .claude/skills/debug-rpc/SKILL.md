---
name: debug-rpc
description: Investigate a failing contract call, transaction, or event fetch (revert reasons, wrong encoding, provider limits, rate limits, unexpected nil). Use when a Vium user reports "it does not work against the node".
---
...
```ruby
Vium.config.logger = Logger.new($stdout, level: Logger::DEBUG)   # logs every method + params + result
```
Keys are masked; the debug log is safe to paste. Compare `data` against `contract.encode_function_data(...)`.

## 2. Reproduce offline

Copy the real response into a `Vium::Connectors::Stub` and write the failing spec first. Stub values:
...
| `HttpError` 429 after retries | rate limit; raise `polling_interval`, lower watcher count, check `Vium.watchers` for duplicates | connector `retries`/`retry_delay` |
...
```ruby
Vium.configure { |c| c.connector = Vium::Connectors::Http.new; c.chain = :base }   # public RPC, no key
Vium.client.block_number
```
