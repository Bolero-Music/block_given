---
name: add-chain
description: Register a new EVM network in BlockGiven::Chains (id, RPC url, explorer, Alchemy slug) or fix an existing chain definition.
---

# Add a chain

Chains are constants in `lib/block_given/chain.rb`, resolved by `Chains.resolve(:symbol | "network-slug" | id)`.
...
4. Spec in `spec/block_given/chains_spec.rb`: `resolve` by symbol and id, `alchemy_network`, explorer url.
5. README "Chains" list + CHANGELOG.

Applications can also define chains inline with `BlockGiven::Chain.new(...)` without touching the gem; prefer
