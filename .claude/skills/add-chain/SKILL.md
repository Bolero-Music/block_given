---
name: add-chain
description: Register a new EVM network in Vium::Chains (id, RPC url, explorer, Alchemy slug) or fix an existing chain definition.
---

# Add a chain

Chains are constants in `lib/vium/chain.rb`, resolved by `Chains.resolve(:symbol | "network-slug" | id)`.
...
4. Spec in `spec/vium/chains_spec.rb`: `resolve` by symbol and id, `alchemy_network`, explorer url.
5. README "Chains" list + CHANGELOG.

Applications can also define chains inline with `Vium::Chain.new(...)` without touching the gem; prefer
