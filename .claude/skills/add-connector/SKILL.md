---
name: add-connector
description: Implement a new JSON-RPC connector/transport (Infura, QuickNode, WebSocket, local node) or change HTTP retry/redaction behaviour. Use when touching lib/vium/connectors.
---

# Add a connector

A connector is a transport: `request(method, params, chain:)` returns the JSON-RPC `result` or raises
`Vium::RpcError`. Everything above it (Client, Contract) stays unchanged.
...
5. **Specs** in `spec/vium/connectors/<name>_spec.rb` using WebMock (`stub_request`): endpoint per chain,
