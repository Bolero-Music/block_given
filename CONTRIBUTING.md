...
```bash
git clone git@github.com:Bolero-Music/vium.git && cd vium
...
- **No network in specs.** Use `Vium::Connectors::Stub` for RPC responses and WebMock for the HTTP layer.
...
4. Make sure `bundle exec rspec`, `bundle exec rubocop` and `gem build vium.gemspec` pass.
...
See the "Versioning & releases" section of the README. In short: bump `lib/vium/version.rb`, move the
