...
RSpec.describe UncleBlockGiven::Client do
  let(:stub) { build_stub }
  ...
  it "falls back when eth_maxPriorityFeePerGas is unsupported" do
    stub.stub("eth_maxPriorityFeePerGas", UncleBlockGiven::RpcError.new("unsupported", code: -32_601))
  ...
  describe "#wait_for_transaction_receipt" do
    ...
    it "polls until the receipt is available" do
      stub.stub("eth_getTransactionReceipt", UncleBlockGiven::Connectors::Stub.sequence(nil, nil, receipt))
      result = client.wait_for_transaction_receipt(hash)
      expect(result).to be_a(UncleBlockGiven::Receipt)
    ...
    it "waits for confirmations" do
      ...
      stub.stub("eth_blockNumber", UncleBlockGiven::Connectors::Stub.sequence("0x10", "0x11", "0x12"))
    ...
    it "times out" do
      ...
      expect { client.wait_for_transaction_receipt(hash, timeout: 0.05) }.to raise_error(UncleBlockGiven::TimeoutError)
    end
  end

  describe "#watch_block_number" do
    it "yields new block numbers in the background and stops" do
      stub.stub("eth_blockNumber", UncleBlockGiven::Connectors::Stub.sequence("0x10", "0x10", "0x12", "0x12"))
      seen = []
  ...
  describe "#watch_logs" do
    it "polls new block ranges and yields log batches" do
      stub.stub("eth_blockNumber", UncleBlockGiven::Connectors::Stub.sequence("0x10", "0x12", "0x12", "0x13"))
      stub.stub("eth_getLogs", ->(params) { [{ "blockNumber" => params.first[:toBlock], "topics" => [], "data" => "0x" }] })
      batches = []
    ...
    it "keeps polling after an error and reports it" do
      stub.stub("eth_blockNumber", UncleBlockGiven::Connectors::Stub.sequence("0x10", "0x11", "0x12"))
      stub.stub("eth_getLogs", UncleBlockGiven::Connectors::Stub.sequence(UncleBlockGiven::RpcError.new("flaky"), []))
      errors = []
