...
RSpec.describe UncleBlockGiven::SignedTransaction do
  let(:stub) { build_stub }
  ...
  it "knows its hash and nonce before any broadcast" do
    ...
    expect(signed.hash).to eq(UncleBlockGiven::Utils.keccak256(signed.raw))
  ...
  it "warns when the node returns another hash but keeps the local one" do
    log = StringIO.new
    UncleBlockGiven.config.logger = Logger.new(log)
  ...
  describe "#replacement" do
    ...
    it "refuses multipliers the node would reject" do
      expect { signed.replacement(fee_multiplier: 1.05) }
        .to raise_error(UncleBlockGiven::InvalidArgumentError, /fee_multiplier must be >= 1.1/)
    end
  end
  ...
  describe ".from_raw" do
    ...
    it "rebuilds a legacy transaction with calldata" do
      ...
      rebuilt = described_class.from_raw(UncleBlockGiven::Utils.strip_hex(legacy.raw), wallet: wallet)
    ...
    it "refuses bytes signed by another key or undecodable bytes" do
      other = UncleBlockGiven::Wallet.generate
      expect { described_class.from_raw(signed.raw, wallet: other) }
        .to raise_error(UncleBlockGiven::InvalidArgumentError, /not by wallet #{other.address}/)
      expect { described_class.from_raw("0x02deadbeef", wallet: wallet) }
        .to raise_error(UncleBlockGiven::InvalidArgumentError, /cannot decode/)
    end
  end
  ...
  it "refuses to replace a transaction rebuilt without fee params" do
    ...
    expect { bare.replacement }.to raise_error(UncleBlockGiven::InvalidArgumentError, /without fee params/)
  end
  ...
  it "serializes for persistence and hides nothing sensitive" do
    ...
    expect(signed.inspect).to eq("#<UncleBlockGiven::SignedTransaction #{signed.hash} nonce=5 to=#{OTHER_ADDRESS}>")
    expect(signed.inspect).not_to include("ac0974")
    expect(signed.to_s).to eq(signed.hash)
    expect(signed).to eq(UncleBlockGiven::SignedTransaction.new(raw: signed.raw, params: {}, wallet: wallet))
  end
end
