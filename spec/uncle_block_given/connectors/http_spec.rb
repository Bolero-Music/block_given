...
RSpec.describe UncleBlockGiven::Connectors::Http do
  ...
  it "falls back to the chain's public RPC when no url is given" do
    expect(described_class.new.endpoint(UncleBlockGiven::Chains::BASE)).to eq("https://mainnet.base.org")
    expect { described_class.new.endpoint(UncleBlockGiven::Chain.new(id: 1, name: "x")) }.to raise_error(UncleBlockGiven::ConfigurationError)
    expect(described_class.new.inspect).to eq("#<UncleBlockGiven::Connectors::Http url=chain default>")
  end

  it "keeps only scheme and host of the url in inspect and errors" do
    expect(connector.inspect).to eq('#<UncleBlockGiven::Connectors::Http url="https://mainnet.infura.io/…">')
    ...
    expect { connector.request("eth_chainId", [], chain: UncleBlockGiven::Chains::MAINNET) }.to raise_error(UncleBlockGiven::HttpError) do |e|
  ...
  it "raises HttpError on connection failures and invalid JSON" do
    ...
    expect { connector.request("eth_chainId", [], chain: nil) }.to raise_error(UncleBlockGiven::HttpError, /ECONNREFUSED/)

    stub_request(:post, url).to_return(body: "<html>")
    expect { connector.request("eth_chainId", [], chain: nil) }.to raise_error(UncleBlockGiven::HttpError, /invalid JSON/)
  end
end
