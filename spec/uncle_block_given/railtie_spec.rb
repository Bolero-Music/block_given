...
RSpec.describe "UncleBlockGiven::Railtie", if: defined?(Rails::Railtie) do
  it "is registered and defaults the logger to Rails.logger once the app boots" do
    expect(defined?(UncleBlockGiven::Railtie)).to be_truthy

    logger = Logger.new(nil)
    root = Dir.mktmpdir("uncle_block_given-rails")
    app_class = Class.new(Rails::Application) do
      config.root = root
      config.eager_load = false
      config.logger = logger
      config.secret_key_base = "uncle_block_given-test"
      config.active_support.deprecation = :silence
    end
    stub_const("UncleBlockGivenTestApp::Application", app_class) # Rails derives the app name from the constant
    app_class.initialize!

    # Rails 7.1+ wraps the configured logger in an ActiveSupport::BroadcastLogger
    expect(UncleBlockGiven.config.logger).to equal(Rails.logger)
  ensure
