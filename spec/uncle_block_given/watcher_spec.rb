...
RSpec.describe UncleBlockGiven::Watcher do
  ...
  it "registers running watchers under a unique id and unregisters them on stop" do
    ...
    expect(UncleBlockGiven.watchers.map(&:id)).to contain_exactly(a.id, b.id)
    expect(described_class.find(a.id)).to equal(a)
    expect(Thread.list.map(&:name)).to include("uncle_block_given:#{a.id}")
  ...
  it "accepts a custom id and refuses duplicates while running" do
    ...
    expect { described_class.new(interval: 0.01, id: "usdc-deposits") { nil }.start }
      .to raise_error(UncleBlockGiven::InvalidArgumentError, /already running/)
    expect { described_class.find!("nope") }.to raise_error(UncleBlockGiven::InvalidArgumentError, /running: usdc-deposits/)
  ...
  it "exposes diagnostics in to_h" do
    ...
    watcher = described_class.new(interval: 0.001, name: "diag") do |w|
      ticks += 1
      w.cursor = ticks
      raise UncleBlockGiven::RpcError, "flaky" if ticks == 2
    end.start
    wait_until { ticks >= 3 }
    snapshot = watcher.to_h
    expect(snapshot).to include(id: watcher.id, name: "diag", status: :running, interval: 0.001, thread: "uncle_block_given:#{watcher.id}")
    expect(snapshot[:ticks]).to be >= 3
    expect(snapshot[:cursor]).to be >= 3
    expect(snapshot[:last_error]).to eq("UncleBlockGiven::RpcError: flaky")
    expect(snapshot[:started_at]).to be_a(Time)
    expect(watcher.inspect).to include(watcher.id, "running", "last_error=UncleBlockGiven::RpcError")
  end
  ...
  it "stops everything with stop_all and on UncleBlockGiven.reset!" do
  ...
  it "names event watchers after the contract and event and honours id:" do
    ...
    anonymous = UncleBlockGiven.client.watch_block_number { |_n| nil }
