...
require "rails/all" if ENV["RAILS_COMPAT"] # Rails must be loaded before block_given for the railtie
require "block_given"
...
module SpecHelpers
  ...
  def word(value)
    ...
    when String then "0x#{BlockGiven::Utils.strip_hex(value).rjust(64, '0')}"
    end
  end
  ...
  def build_stub(extra = {})
    BlockGiven::Connectors::Stub.new(
      {
        "eth_chainId" => "0x2105",
  ...
  def configure_block_given(stub, chain: :base)
...
RSpec.configure do |config|
  ...
  config.before { BlockGiven.reset! }
  config.after { BlockGiven.reset! }
end
