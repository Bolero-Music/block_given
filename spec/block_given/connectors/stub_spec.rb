...
RSpec.describe BlockGiven::Connectors::Stub do
  ...
  it "raises for unknown methods and raises stubbed exceptions" do
    stub = described_class.new("eth_call" => BlockGiven::RpcError.new("boom"))
    expect { stub.request("eth_chainId") }.to raise_error(BlockGiven::RpcError, /no stub/)
    expect { stub.request("eth_call") }.to raise_error(BlockGiven::RpcError, "boom")
  end

  it "falls back to a handler block" do
    stub = described_class.new { |method, _params, chain| "#{method}@#{chain.id}" }
    expect(stub.request("eth_chainId", [], chain: BlockGiven::Chains::BASE)).to eq("eth_chainId@8453")
  end
end
