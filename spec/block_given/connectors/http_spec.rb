...
RSpec.describe BlockGiven::Connectors::Http do
  ...
  it "falls back to the chain's public RPC when no url is given" do
    expect(described_class.new.endpoint(BlockGiven::Chains::BASE)).to eq("https://mainnet.base.org")
    expect { described_class.new.endpoint(BlockGiven::Chain.new(id: 1, name: "x")) }.to raise_error(BlockGiven::ConfigurationError)
    expect(described_class.new.inspect).to eq("#<BlockGiven::Connectors::Http url=chain default>")
  end

  it "keeps only scheme and host of the url in inspect and errors" do
    expect(connector.inspect).to eq('#<BlockGiven::Connectors::Http url="https://mainnet.infura.io/…">')
    ...
    expect { connector.request("eth_chainId", [], chain: BlockGiven::Chains::MAINNET) }.to raise_error(BlockGiven::HttpError) do |e|
  ...
  it "raises HttpError on connection failures and invalid JSON" do
    ...
    expect { connector.request("eth_chainId", [], chain: nil) }.to raise_error(BlockGiven::HttpError, /ECONNREFUSED/)

    stub_request(:post, url).to_return(body: "<html>")
    expect { connector.request("eth_chainId", [], chain: nil) }.to raise_error(BlockGiven::HttpError, /invalid JSON/)
  end
end
