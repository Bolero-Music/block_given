...
RSpec.describe BlockGiven::Transaction do
  let(:hash) { "0x#{'ab' * 32}" }
  let(:stub) { build_stub }
  let(:client) { BlockGiven::Client.new(chain: :base, connector: stub, polling_interval: 0.01, timeout: 1) }
  ...
  it "waits and raises on reverted receipts with wait!" do
    ...
    expect { transaction.wait! }.to raise_error(BlockGiven::TransactionRevertedError) { |e| expect(e.receipt.block_number).to eq(16) }
  end
