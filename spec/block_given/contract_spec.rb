...
RSpec.describe BlockGiven::Contract do
  let(:stub) { build_stub }
  ...
  describe "class-level DSL" do
    it "declares abi, default address and chain, inherited by subclasses" do
      ...
      expect(sub.chain).to eq(BlockGiven::Chains::BASE)
    end

    it "resolves abi_file against BlockGiven.config.abi_path" do
      BlockGiven.configure { |c| c.abi_path = FIXTURES }
      klass = Class.new(described_class) { abi_file "erc20.json" }
      expect(klass.functions.map(&:name)).to include("transfer")
      expect { Class.new(described_class) { abi_file "missing.json" } }.to raise_error(BlockGiven::AbiError, /not found/)
    end

    it "requires an ABI and an address" do
      expect { Class.new(described_class).new(address: USDC_BASE) }.to raise_error(BlockGiven::AbiError)
      expect { TestERC20.new }.to raise_error(BlockGiven::InvalidArgumentError, /address is required/)
    end
  ...
  describe "reads" do
    it "calls view functions and decodes the result" do
      ...
      expect(stub.calls_for("eth_call").last)
        .to eq([{ to: USDC_BASE, data: "0x70a08231#{BlockGiven::Utils.strip_hex(word(OTHER_ADDRESS)).downcase}",
    ...
    it "decodes revert reasons with the contract ABI" do
      error = usdc.interface.error_by_selector(BlockGiven::Utils.keccak256("ERC20InsufficientBalance(address,uint256,uint256)")[0, 10])
  ...
  describe "writes" do
    ...
    it "requires a wallet" do
      expect { TestERC20.at(USDC_BASE).transfer(OTHER_ADDRESS, 1) }.to raise_error(BlockGiven::WalletRequiredError)
      expect { TestERC20.at(USDC_BASE).prepare_write(:transfer, OTHER_ADDRESS, 1) }.to raise_error(BlockGiven::WalletRequiredError)
    end
    ...
    it "prepares a signed transaction whose hash is known before broadcasting" do
      ...
      expect { usdc.prepare_write(:transfer, OTHER_ADDRESS, 1, tx: { value: 1 }) }.to raise_error(BlockGiven::InvalidArgumentError, /not payable/)
    end
    ...
    it "decodes custom errors raised while broadcasting a prepared transaction" do
      ...
      expect { signed.broadcast }.to raise_error(BlockGiven::ContractRevertError) do |e|
        expect(e.error_name).to eq("ERC20InsufficientBalance")
    ...
    it "refuses to send value to non payable functions and unknown tx options" do
      expect { usdc.transfer(OTHER_ADDRESS, 1, tx: { value: 1 }) }.to raise_error(BlockGiven::InvalidArgumentError, /not payable/)
      expect { usdc.transfer(OTHER_ADDRESS, 1, tx: { gasLimit: 1 }) }.to raise_error(BlockGiven::InvalidArgumentError, /unknown tx option/)
    end
  ...
  describe "events" do
    ...
    it "watches events with polling" do
      stub.stub("eth_blockNumber", BlockGiven::Connectors::Stub.sequence("0x10", "0x11", "0x11"))
      stub.stub("eth_getLogs", [log])
      seen = []
