...
RSpec.describe Vium::Abi::Interface do
  let(:abi) do
    [
  ...
  it "parses functions, events, errors and constructor" do
    ...
    expect(interface.errors.first.selector).to eq(Vium::Utils.keccak256("Unauthorized(address)")[0, 10])
  ...
  it "parses artifacts and JSON strings" do
    ...
    expect { described_class.parse("nope") }.to raise_error(Vium::AbiError)
  end
  ...
  describe "#function" do
    ...
    it "resolves overloads by arity or keyword names" do
      ...
      expect { interface.function(:safe_mint) }.to raise_error(Vium::FunctionNotFoundError, /overload/)
      expect { interface.function(:nope) }.to raise_error(Vium::FunctionNotFoundError, /known:/)
    end
  end
  ...
  describe "functions" do
    it "encodes positional and keyword arguments" do
      ...
      expected = "0xa9059cbb#{Vium::Utils.strip_hex(abi_encode(%w[address uint256], [OTHER_ADDRESS, 1_000_000]))}"
    ...
    it "explains bad keyword arguments" do
      ...
      expect { fn.encode([], { to: OTHER_ADDRESS, value: 1 }) }
        .to raise_error(Vium::InvalidArgumentError, /unknown argument\(s\) value \(expected to, amount\)/)
      expect { fn.encode([], { to: OTHER_ADDRESS }) }.to raise_error(Vium::InvalidArgumentError, /missing argument\(s\) amount/)
      expect { fn.encode([OTHER_ADDRESS], { amount: 1 }) }.to raise_error(Vium::InvalidArgumentError, /mix/)
    end

    it "rejects fractional numbers, bad addresses and overflows" do
      fn = interface.function(:transfer)
      expect { fn.encode([OTHER_ADDRESS, 1.5]) }.to raise_error(Vium::InvalidArgumentError, /parse_units/)
      expect { fn.encode(["0x12", 1]) }.to raise_error(Vium::InvalidAddressError)
      expect { fn.encode([OTHER_ADDRESS, 2**256]) }.to raise_error(Vium::AbiError)
    end

    it "accepts wallets as addresses" do
      wallet = Vium::Wallet.new(private_key: TEST_PRIVATE_KEY)
      expect(interface.function(:transfer).encode([wallet, 1])).to include(Vium::Utils.strip_hex(TEST_ADDRESS).downcase)
    end
    ...
    it "raises a clear error on empty return data" do
      expect { interface.function(:transfer).decode_output("0x") }.to raise_error(Vium::AbiError, /empty data/)
    end
  end
  ...
  describe "events" do
    ...
    it "keeps the hash for indexed dynamic types" do
      ...
      log = { topics: [named.topic, Vium::Utils.keccak256("hello")], data: abi_encode(%w[uint256 address], [1, OTHER_ADDRESS]) }
      expect(named.decode(log).args).to eq(label: Vium::Utils.keccak256("hello"), id: 1, who: OTHER_ADDRESS)
    end
    ...
    it "encodes topic filters with wildcards and OR lists" do
      ...
      expect(interface.event(:named).encode_topics(label: "hello")).to eq([interface.event(:named).topic, Vium::Utils.keccak256("hello")])
      expect { transfer.encode_topics(value: 1) }.to raise_error(Vium::InvalidArgumentError, /not an indexed/)
    end
  end
  ...
  describe "custom errors" do
    it "decodes custom error data" do
      ...
      data = error.selector + Vium::Utils.strip_hex(abi_encode(["address"], [OTHER_ADDRESS]))
