...
RSpec.describe Vium::Wallet do
  ...
  it "derives the address from the private key" do
    ...
    expect(described_class.new(private_key: Vium::Utils.strip_hex(TEST_PRIVATE_KEY)).address).to eq(TEST_ADDRESS)
  end

  it "rejects malformed keys and hides the key in inspect" do
    expect { described_class.new(private_key: "0x1234") }.to raise_error(Vium::InvalidArgumentError)
  ...
  it "generates random wallets" do
    ...
    expect(Vium::Utils.address?(a.address)).to be true
  end
  ...
  describe "#prepare_transaction" do
    ...
    it "rejects fractional wei" do
      expect { wallet.prepare_transaction(to: OTHER_ADDRESS, value: 1.5) }.to raise_error(Vium::InvalidArgumentError)
    end
  end
  ...
  describe "#send_transaction" do
    ...
    it "signs an EIP-1559 transaction the node can recover" do
      ...
      expect(decoded.sender.downcase).to eq(Vium::Utils.strip_hex(TEST_ADDRESS).downcase)
      expect(decoded.destination.downcase).to eq(Vium::Utils.strip_hex(OTHER_ADDRESS).downcase)
